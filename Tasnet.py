...
class ConvTasNet(torch.nn.Module):
    ...
    *Conv-TasNet: Surpassing Ideal Time–Frequency Magnitude Masking for Speech Separation*
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        #feats = self.conv(feats).squeeze(1)
        feats = torch.sum(feats, 1)
...
if __name__ == '__main__':
    with torch.no_grad():        
        model = ConvTasNet(num_sources=1,num_channels = 3 ,enc_kernel_size = 256, enc_num_feats = 256, msk_kernel_size = 3, msk_num_feats= 128, msk_num_hidden_feats = 256, msk_num_layers = 8, msk_num_stacks = 3)
